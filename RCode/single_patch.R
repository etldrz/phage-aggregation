...
quantileGrid <- function(final_file, nA, alpha=NA, theta=NA, p=NA, lambda=NA, omega=NA,
                        reps=500000, seq_step_size, margin=0.05, colors){
  ...
  for(x_val in xs){
    ...
    for(y_val in ys){
      ...
      else if(floatMatch(perc, upper_margin) | perc > upper_margin)
        grid <- fillLocation(grid, x_val, y_val, colors, FALSE)
      else if(floatMatch(perc, margin) | perc < margin)
...
plotQuantileGrid <- function(grid, colors, nA, alpha, theta, p, omega, lambda){
  ...
  legend("topright", inset=c(-0.6, 0), legend=c("WG prom.", "WS prom.", "WS > nA"),
         col=colors[2:4], pch=15)
