...
# paper.base.plot <- "C:\\Users\\Evan\\Desktop\\repos\\phage-aggregation\\data\\bootstrapped\\alpha=0.1, theta=0.2, p=0.5, lambda=0.01, omega=0.txt"
# 
# data <- read.table(paper.base.plot, header=TRUE, sep=",")

r.star <- rStar(paper.base.plot, .1)
...
abline(v=r.star$r.star.mean*(burst.size.A - 1) + 1, lty='dotted', col='black', lwd=1.5)

legend('topleft', legend=c("Specialist", "Generalist"), 
       col=c('firebrick', 'darkorchid3'), border='white', pch=16, bty='n')
