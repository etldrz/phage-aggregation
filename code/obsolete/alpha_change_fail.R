...
#plot_file <- "C:\\Users\\Evan\\Desktop\\R Shit\\phageAggregation\\plottingData\\alphaChangeGoodError.TXT"
...
slopeEqual <- function(min_x, min_wg, min_ws, max_x, max_wg, max_ws){
  ...
  # Take care of this outside of this function, when you call the main function here.
...
replaceFitness <- function(outcomes, nB, lambda, alpha) {
...
zeroHunter <- function(fitness){
  ...
  r_star_mean <- rep(mean(fitness$r_star), nrow(fitness))
...
equilibriumSimulation <- function(nA, lyse, alpha, theta, p,
  ...
  for(i in 1:treatments){
    ...
    bootstrap_fitness_g <- replaceFitness(g_outcomes, nB=nB, lambda=lambda, alpha=alpha)
    bootstrap_fitness_s <- replaceFitness(s_outcomes, nB=nB, lambda=lambda, alpha=alpha)
    ...
    if(b) {
      ...
      legend <- c('x_lower', 'bt_ws_lower','bt_wg_lower','x_upper',
                  ...
                  'upper_quantile', 'r_star', 'r_star_mean',
                  'changing_value', 'lambda', 'prediction')
...
for (i in 1:iterations) {
 ...
 print(paste("Done with ", i, "out of ", iterations))
}
...
for(i in alpha_change){
  ...
  fitness$r_star[curr_lambda1] <- changed_lambda1$r_star
  ...
  fitness$r_star[curr_lambda2] <- changed_lambda2$r_star
